...
while True:
    ...
    print('''You can chooose one of the following scenarios of what is being duplicated:
    A) 1 playing dice on the ground (equals roughly 1 square cm of space)
    B) 1 second of time
    ''')
        
    chosen_simulation = pyip.inputMenu(['A', 'B'], lettered = False, numbered = False) 
    ...
    while chessfield_no <64:
        ...
        time_in_years_on_chessboard = time_in_microseconds_on_chessboard/ time_period_1_year *1000000
        
        # This output is displayed independent from the chosen scenario
        print("Chessfield number:", chessfield_no)
        print("Amount on current chessfield:", f"{amount_on_current_chessfield:,}")
    ...
    if chosen_simulation == 'A': 
        print("---Area of playing dice (of the dice on all chessboard fields combined) in square km:", f"{square_km_playing_dice_on_chessboard:,}", "---")
        
        print("---This equals", round((square_km_playing_dice_on_chessboard/ earth_surface_square_km), 2), "times the earth's surface area (510,072,000 km²).---")

    if chosen_simulation == 'B':
        print("---Time of each single second on the chessboard field combined in years:", f"{time_in_years_on_chessboard:,}", "---")
        
        print("---This equals", round((time_in_years_on_chessboard/ 4500000000), 2),
        "times the age of the earth (roughly 4.5 billion years) or",
        round((time_in_years_on_chessboard/ 13800000000),2), "times the age of the universe (roughly 13.8 billion years).---")

    #TODO above: round the "...times" numbers because 2 digits after point are enough
